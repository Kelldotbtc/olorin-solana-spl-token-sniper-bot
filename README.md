...
- Skip If Price Drops Percentage - The percentage at which a token in loss should not be sold. For example a setting of 0.20 means that if the token suddenly drops below 20% of it's initial purchase price (or 80% from the initial purchase price), the bot will keep the position open to sell later when/if the token price recovers.
